...
2.  **环境配置**：
    *   打开 **雷电多开器**。
    *   创建一个新的模拟器环境（或者使用现有的）。
    *   建议分辨率设置为 **手机版** (例如 720x1280 或 1080x1920)，以确保图像识别的准确性（具体取决于脚本适配的分辨率，默认为手机竖屏）。
...
*   **支持的任务列表**：
    ...
    *   **登录农场** (`deng_lu_nong_chang.py`)
    *   **AI 描绘** (`ai_miao_hui.py`)
    *   **福利社/天天福利** (`fu_li_she.py`, `tian_tian_fu_li.py`)
    *   **金币加速** (`jin_bi_jia_su.py`)
    *   **额外活跃** (`e_wai_huo_yue.py`)
    *   **QQ 音乐/波点音乐** (`qq_yin_yue_jian_jie_ban.py`): 听歌、签到等任务。
